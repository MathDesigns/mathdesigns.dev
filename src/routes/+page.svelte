<script lang="ts">
	...
	import { Mail, Terminal, Github, Copy, Check } from '@lucide/svelte';
	...
	function copyEmail() {
		navigator.clipboard
			.writeText('math@mathdesigns.dev')
			.then(() => {
				isCopied = true;
		...
		>
			...
			>
				<span class="relative flex h-1.5 w-1.5">
					<span
						class="bg-primary absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 motion-reduce:hidden"
					></span>
	...
	<div class="mx-auto flex max-w-3xl flex-col items-center justify-center text-center">
		...
		>
			...
			<Button
				...
				variant="outline"
				size="lg"
				class="h-14 w-full cursor-pointer rounded-full bg-transparent px-8 text-base hover:bg-white/10 hover:text-white sm:w-auto"
			>
				{#if isCopied}
